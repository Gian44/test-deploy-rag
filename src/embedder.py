...
def chromadbClient():
    ...
    client.create_collection(COLLECTION_NAME, get_or_create=True)
    client.create_collection(PROCESSED_IDS_COLLECTION, get_or_create=True)
...
def storeInChromaDB(chunks, client):
    try:
        ...
        updateProcessedIDs(newIds, client)
...
def formatHAttachments(attachments):
    if not attachments:
        return ""
    ...
    for att in attachments:
        ...
        if att.get('path'):
            parsed = parser.from_file(att['path'])
...
def generateHumanReadableContent(item):
    ...
    attachments = formatHAttachments(item.get('attachments', []))
    ...
    if attachments:
        content += f"There are {len(item.get('attachments', []))} attachments in this issue:\n{attachments}\n"
...
def extractContent(item):
    content = {
        ...
        "attachments": formatAttachments(item.get('attachments', [])),
...
def formatAttachments(attachments):
    ...
    return "\n".join([formatAttachment(att) for att in attachments if 'path' in att and os.path.exists(att['path'])])

def formatAttachment(att):
    fields = []
    if att['filename']:
        fields.append(f"Filename: {att['filename']}")
    if att['description'] or att['description'] == "":
        fields.append(f"Description: {att['description']}")
    if att['path']:
        parsed = parser.from_file(att['path'])
        fields.append(f"Parsed Content: {preprocessText(parsed['content'].strip())}")
        # fields.append(f"Content: {parsed['content'].strip()}")
    if att['content']:
...
def readProcessedIDs(client):
    vectorstore = Chroma(
        client=client,
        embedding_function=EMBEDDING,
        persist_directory=PERSIST_DIR,
        collection_name=PROCESSED_IDS_COLLECTION
    )
    results = vectorstore.get()["documents"]
    print("Processed IDs: ", results)
    return set([int(id) for id in results])

def updateProcessedIDs(newIds, client):
    vectorstore = Chroma(
        client=client,
        embedding_function=EMBEDDING,
        persist_directory=PERSIST_DIR,
...
def main():
    ...
    with open("raw_data.txt", "w+") as file:
        print(json.dumps(rawData, indent=2), file=file)

    if rawData is None:
        print("Failed to extract data from SQLite database.")
        return

    newDataEmbedded = embedDocs(client, rawData)
    
    if newDataEmbedded:
        print("New data embedded, fetching vectorstore and rawDataDocuments...")
    else:
